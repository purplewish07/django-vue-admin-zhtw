...
<script>
	 ...
	 * @property {String} html 富文本數據
	 * @property {Boolean} autopause 是否在播放一個視頻時自動暫停其他視頻
	 * @property {Boolean} autoscroll 是否自動給所有表格添加一個滾動層
	 * @property {Boolean} autosetTitle 是否自動將 title 標籤中的內容設置到頁面標題
	 * @property {Number} compress 壓縮等級
	 * @property {String} domain 圖片、視頻等鏈接的主域名
	 * @property {Boolean} lazyLoad 是否開啟圖片懶加載
	 * @property {String} loadingImg 圖片加載完成前的佔位圖
	 * @property {Boolean} selectable 是否開啟長按複製
	 * @property {Object} tagStyle 標籤的默認樣式
	 * @property {Boolean} showWithAnimation 是否使用漸顯動畫
	 * @property {Boolean} useAnchor 是否使用錨點
	 * @property {Boolean} useCache 是否緩存解析結果
	 * @event {Function} parse 解析完成事件
	 * @event {Function} load dom 加載完成事件
	 * @event {Function} ready 所有圖片加載完畢事件
	 * @event {Function} error 錯誤事件
	 * @event {Function} imgtap 圖片點擊事件
	 * @event {Function} linkpress 鏈接點擊事件
	 * @author JinYufeng
	 * @version 20201029
	 * @listens MIT
	 */
	export default {
		...
		methods: {
			...
			_handleHtml(html, append) {
				if (!append) {
					// 處理 tag-style 和 userAgentStyles
