<template>
  ...
  <div >
    <el-button v-show="listQuery.type" type="primary" icon="el-icon-plus" @click="handleAddDict" v-if="checkPermission(['dic_create'])" size="small">新增</el-button>
        ...
        >
          ...
          <el-table-column label="是否有效">
            <template slot-scope="scope">
              ...
              <el-tag type="danger" v-else>無效</el-tag>
              </template>
          </el-table-column>
        ...
        <p v-show="!listQuery.type">點擊左側類型查看字典</p>
        <!-- <pagination
    ...
    <el-dialog :visible.sync="dgV1" :title="dgT1==='edit'?'編輯字典類型':'新建字典類型'">
      <el-form ref="Form1" :model="dicttype" label-width="80px" label-position="right" :rules="rule1">
        <el-form-item label="名稱" prop="name">
          <el-input v-model="dicttype.name" placeholder="名稱" />
        </el-form-item>
        <el-form-item label="代號" prop="code">
          <el-input v-model="dicttype.code" placeholder="代號" />
        </el-form-item>
        <el-form-item label="父級" prop="parent">
          <treeselect v-model="dicttype.parent" :multiple="false" :options="dictTypeData" placeholder="父級"/>
        </el-form-item>
      </el-form>
      <span slot="footer">
        <el-button type="danger" @click="dgV1=false">取消</el-button>
        <el-button type="primary" @click="confirm1('Form1')">確認</el-button>
      </span>
    </el-dialog>
    <el-dialog :visible.sync="dgV2" :title="dgT2==='edit'?'編輯字典':'新建字典'">
      <el-form ref="Form2" :model="dict" label-width="80px" label-position="right" :rules="rule2">

        <el-form-item label="編號" prop="code">
          <el-input v-model="dict.code" placeholder="編號" />
        </el-form-item>
        <el-form-item label="名稱" prop="name">
          <el-input type="textarea" v-model="dict.name" placeholder="名稱" />
        </el-form-item>
        <el-form-item label="描述" prop="description">
          <el-input type="textarea" v-model="dict.description" placeholder="描述" />
        </el-form-item>
        <el-form-item label="父級" prop="parent">
          <treeselect v-model="dict.parent" :multiple="false" :options="dictList" placeholder="父級"/>
        </el-form-item>
  ...
  v-model="dict.is_used">
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="confirm2('Form2')">確認</el-button>
      </span>
    </el-dialog>
  </div>
</template>
  ...
  data() {
    return {
      ...
      rule1: {
        ...
        code: [{ required: true, message: '請輸入代號', trigger: 'change' }]
      },
      dgV2: false,
      dgT2: 'new',
      rule2: {
