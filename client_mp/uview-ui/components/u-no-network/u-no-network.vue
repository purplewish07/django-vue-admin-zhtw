<template>
	<view class="u-no-network" v-if="!isConnected" :style="{'z-index': uZIndex}" @touchmove.stop.prevent="() => {}">
		<view class="u-inner">
			...
			<!-- 只有APP平台，才能跳轉設置頁，因為需要調用plus環境 -->
			<!-- #ifdef APP-PLUS -->
			<view class="u-to-setting">
				請檢查網絡，或前往<text class="u-setting-btn" @tap="openSettings">設置</text>
			</view>
			<!-- #endif -->
...
<script>
	/**
	 * noNetwork 無網絡提示
	 * @description 該組件無需任何配置，引入即可，內部自動處理所有功能和事件。
	 * @tutorial https://www.uviewui.com/components/noNetwork.html
	 * @property {String} tips 沒有網絡時的提示語（默認哎呀，網絡信號丟失）
	 * @property {String Number} zIndex 組件的z-index值（默認1080）
	 * @property {String} image 無網絡的圖片提示，可用的src地址或base64圖片
	 * @event {Function} retry 用戶點擊頁面的"重試"按鈕時觸發
	...
	export default {
		...
		props: {
			...
			// 一個z-index值，用於設置沒有網絡這個組件的層次，因為頁面可能會有其他定位的元素層級過高，導致此組件被覆蓋
			zIndex: {
				type: [Number, String],
				default: ''
			},
		...
		data() {
			return {
				isConnected: true, // 是否有網絡連接
				networkType: "none", // 網絡類型
			}
		},
		computed: {
			uZIndex() {
