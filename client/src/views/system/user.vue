  ...
  <div class="app-container">
    ...
    <span>用戶</span>
        ...
        >
          ...
          <el-table-column align="header-center" label="賬戶">
            <template slot-scope="scope">{{ scope.row.username }}</template>
          </el-table-column>
          <el-table-column align="header-center" label="部門">
            <template
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='edit'?'編輯用戶':'新增用戶'">
      <el-form ref="Form" :model="user" label-width="80px" label-position="right" :rules="rule1">
        ...
        <el-form-item label="所屬部門" prop="dept">
          <treeselect v-model="user.dept" :multiple="false" :options="orgData" placeholder="所屬部門"/>
        </el-form-item>
        <el-form-item label="角色" prop="roles">
          <el-select v-model="user.roles" multiple placeholder="請選擇" style="width:100%">
            <el-option
              v-for="item in roles"
              :key="item.value"
              :label="item.label"
        ...
        <el-form-item label="頭像" prop="dept">
          <el-upload
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="confirm('Form')">確認</el-button>
      </span>
    </el-dialog>
  </div>
</template>
