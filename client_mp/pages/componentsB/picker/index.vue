	...
	<view class="u-demo">
		...
		<view class="u-config-wrap">
			...
			<view class="u-config-item">
				<view class="u-item-title">Picker開關</view>
				<u-subsection vibrateShort :current="status" :list="['顯示', '隱藏']" @change="statusChange"></u-subsection>
			</view>
			<view class="u-config-item">
				<view class="u-item-title">模式選擇</view>
				<u-subsection vibrateShort :list="['單列', '多列', '時間', '地區']" @change="modeChange"></u-subsection>
			</view>
			<view class="u-config-item">
				<view class="u-item-title">默認時間</view>
				<u-subsection vibrateShort :list="['2019-12-11 20:15:35', '2020-02-05 13:09:42']" @change="defaultTimeChange"></u-subsection>
			</view>
			<view class="u-config-item">
				<view class="u-item-title">顯示時分秒</view>
				<u-subsection vibrateShort :list="['顯示', '隱藏']" @change="minSecChange"></u-subsection>
			</view>
			<view class="u-config-item">
				<view class="u-item-title">默認地區</view>
				<u-subsection vibrateShort :list="['廣東-深圳-寶安', '海南-三亞-海棠']" @change="defaultRegionChange"></u-subsection>
			</view>
		</view>
	</view>
</template>
	...
	methods: {
		...
		modeChange(index) {
			...
			if (this.mode == 'selector') {
				this.range = ['一', '片', '冰', '心', '在', '玉', '壺'];
				this.defaultSelector = [0];
			}
			if (this.mode == 'multiSelector') {
				this.range = [['亞洲', '歐洲'], ['中國', '日本'], ['北京', '上海', '廣州']];
		...
		defaultRegionChange(index) {
			this.defaultRegion = index == 0 ? ['廣東省', '深圳市', '寶安區'] : ['海南省', '三亞市', '海棠區'];
			this.mode = 'region';
			this.show = true;
		},
		minSecChange(index) {
			...
			switch (column) {
				case 0: 
					switch (index) {
						case 0:
							this.range[1] = ['中國', '日本']
							this.range[2] = ['北京', '上海', '廣州']
							break
						case 1:
							this.range[1] = ['英國', '法國']
							this.range[2] = ['倫敦', '曼徹斯特']
							break
					}
					...
					break
				case 1: //拖動第2列
					switch (this.defaultSelector[0]) { //判斷第一列是什麼
						case 0:
